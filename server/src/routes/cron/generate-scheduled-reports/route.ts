      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>อีเมลนี้ถูกส่งอัตโนมัติจากระบบ medical Platform</p>
            </div>
          </div>
        </body>
      </html>
  ...
  try {
    ...
    for (const scheduledReport of scheduledReports) {
      ...
      try {
        ...
        // Implementation plan:
        // 1. Create a dedicated bucket for reports (e.g., 'scheduled-reports')
        // 2. Generate a unique file path: `reports/${scheduledReport.id}/${fileName}`
        // 3. Convert fileContent to Buffer if it's a string (for CSV)
        // 4. Upload using: supabase.storage.from('scheduled-reports').upload(filePath, buffer, { contentType: mimeType })
        // 5. Get public URL: supabase.storage.from('scheduled-reports').getPublicUrl(filePath)
        // 6. Store fileUrl in the execution record for future reference
        // 7. Consider adding file retention policy (auto-delete old reports after X days)
        // 8. Add error handling for storage failures (should not block email sending)
