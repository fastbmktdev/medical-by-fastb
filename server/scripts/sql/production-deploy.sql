...
CREATE TABLE IF NOT EXISTS public.appointments (
    ...
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    hospital_id UUID REFERENCES public.hospitals(id) ON DELETE CASCADE NOT NULL,
    package_id UUID REFERENCES public.hospital_packages(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.badges (
    ...
    category TEXT NOT NULL CHECK (category IN ('appointment', 'loyalty', 'social', 'learning', 'special')),
...
CREATE TABLE IF NOT EXISTS public.user_streaks (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    streak_type TEXT NOT NULL CHECK (streak_type IN ('appointment', 'login', 'review', 'article_read')),
...
CREATE TABLE IF NOT EXISTS public.leaderboards (
    ...
    name_english TEXT,
    description TEXT,
    leaderboard_type TEXT NOT NULL CHECK (leaderboard_type IN ('points', 'appointments', 'streak', 'monthly', 'all_time')),
...
BEGIN
    ...
    -- hospitals policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'hospitals' AND policyname = 'Anyone can view approved hospitals') THEN
        CREATE POLICY "Anyone can view approved hospitals" ON public.hospitals FOR SELECT USING (status = 'approved' OR auth.uid() = user_id OR is_admin());
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'hospitals' AND policyname = 'Partners can create hospitals') THEN
        CREATE POLICY "Partners can create hospitals" ON public.hospitals FOR INSERT WITH CHECK (is_partner());
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'hospitals' AND policyname = 'Owners can update own hospitals') THEN
        CREATE POLICY "Owners can update own hospitals" ON public.hospitals FOR UPDATE USING (auth.uid() = user_id OR is_admin());
    END IF;

    -- hospital packages policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'hospital_packages' AND policyname = 'Anyone can view active packages') THEN
        CREATE POLICY "Anyone can view active packages" ON public.hospital_packages FOR SELECT USING (is_active = true OR owns_hospital(hospital_id) OR is_admin());
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'hospital_packages' AND policyname = 'hospital owners can manage packages') THEN
        CREATE POLICY "hospital owners can manage packages" ON public.hospital_packages FOR ALL USING (owns_hospital(hospital_id) OR is_admin());
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'appointments' AND policyname = 'Users can view own appointments') THEN
        CREATE POLICY "Users can view own appointments" ON public.appointments FOR SELECT USING (auth.uid() = user_id OR owns_hospital(hospital_id) OR is_admin());
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'appointments' AND policyname = 'Users can create appointments') THEN
        CREATE POLICY "Users can create appointments" ON public.appointments FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'appointments' AND policyname = 'Owners can update appointments') THEN
        CREATE POLICY "Owners can update appointments" ON public.appointments FOR UPDATE USING (owns_hospital(hospital_id) OR is_admin());
    END IF;
END $$;
...
VALUES ('hospital-images', 'hospital-images', true, NOW(), NOW())
ON CONFLICT (id) DO UPDATE SET public = true, updated_at = NOW();

-- Storage policies (with IF NOT EXISTS to prevent duplicate errors)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Anyone can view hospital images') THEN
        CREATE POLICY "Anyone can view hospital images" ON storage.objects FOR SELECT USING (bucket_id = 'hospital-images');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Authenticated users can upload hospital images') THEN
        CREATE POLICY "Authenticated users can upload hospital images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'hospital-images' AND auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Users can update own hospital images') THEN
        CREATE POLICY "Users can update own hospital images" ON storage.objects FOR UPDATE USING (bucket_id = 'hospital-images' AND auth.uid()::text = (storage.foldername(name))[1]);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'Users can delete own hospital images') THEN
        CREATE POLICY "Users can delete own hospital images" ON storage.objects FOR DELETE USING (bucket_id = 'hospital-images' AND auth.uid()::text = (storage.foldername(name))[1]);
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '============================================================================';
    RAISE NOTICE 'Email: medicalhospital@gmail.com';
    ...
    RAISE NOTICE '   SELECT setup_main_admin(''medicalhospital@gmail.com'');';
    RAISE NOTICE '';
...
BEGIN
    ...
    -- Check storage
    SELECT EXISTS(SELECT 1 FROM storage.buckets WHERE id = 'hospital-images') INTO bucket_exists;
    IF bucket_exists THEN
        RAISE NOTICE '📦 Storage bucket: ✅ hospital-images created';
    ELSE
        RAISE NOTICE '📦 Storage bucket: ❌ hospital-images missing';
    END IF;
    ...
    RAISE NOTICE '1. Create auth user for medicalhospital@gmail.com in Supabase Dashboard';
    RAISE NOTICE '2. Run: SELECT setup_main_admin(''medicalhospital@gmail.com'');';
...
-- 👑 MAIN ADMIN USER:
-- Email: medicalhospital@gmail.com
-- Password: ThaiKick2024!@#Admin (auto-generated)
-- Setup: SELECT setup_main_admin('medicalhospital@gmail.com');
...
-- Register new user with complete profile: SELECT register_user_directly('newuser@example.com', NULL, 'authenticated', 'John Doe', 'johndoe', '+66812345678', 'Medical enthusiast');
...
-- Register partner user: SELECT register_user_directly('partner@example.com', NULL, 'partner', 'hospital Owner', 'hospitalowner', '+66987654321', 'hospital Owner and doctor');
