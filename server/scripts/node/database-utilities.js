 ...
 * - hospital slug updates and maintenance
 ...
 *   partners     - Verify partner role promotions and hospital applications
 *   slugs        - Update and verify hospital slugs
...
class DatabaseHealthChecker {
  ...
  async checkStorage() {
    try {
      ...
      const buckets = data || [];
      const gymImagesBucket = buckets.find(bucket => bucket.id === 'hospital-images');
      
      console.log(`📦 Found ${buckets.length} storage bucket(s)`);
      
      if (gymImagesBucket) {
        console.log(`✅ Storage bucket "hospital-images" exists`);
        return { exists: true, buckets: buckets.map(b => b.id) };
      } else {
        console.log(`❌ Storage bucket "hospital-images" does NOT exist`);
  ...
  async runHealthCheck() {
    ...
    const requiredTables = [
      'profiles', 'user_roles', 'hospitals', 'hospital_packages', 
      'appointments', 'payments', 'partner_applications'
    ];
  ...
  generateHealthSummary(results) {
    ...
    if (missingTables.length === 0 && results.storage.exists && tablesWithIssues.length === 0) {
      ...
      if (!results.storage.exists) {
        console.log('   Missing storage: hospital-images bucket');
      }
...
class PartnerRoleManager {
  ...
  async checkPartnerPromotions() {
    ...
    try {
      ...
      // Get all hospitals
      const { data: hospitalsData, error: hospitalsError } = await this.supabase
        .from('hospitals')
        .select('user_id, hospital_name, status, created_at');

      if (hospitalsError) {
        console.error(`❌ Error fetching hospitals: ${hospitalsError.message}`);
        return false;
      }
      ...
      hospitalsData.forEach(hospital => {
        if (!hospitalsByUserId[hospital.user_id]) {
          hospitalsByUserId[hospital.user_id] = [];
        }
        hospitalsByUserId[hospital.user_id].push(hospital);
      });
      ...
      // Check partners with hospital applications
      const partners = rolesData.filter(role => role.role === 'partner');
      const partnersWithHospitals = partners.filter(partner => hospitalsByUserId[partner.user_id]);

      console.log(`\n🏢 Partners with hospital Applications (${partnersWithHospitals.length}):`);
      partnersWithHospitals.forEach(partner => {
        const profile = profilesByUserId[partner.user_id];
        const hospitals = hospitalsByUserId[partner.user_id];
        const hospital = hospitals[0]; // Assuming one hospital per partner
        
        const roleUpdated = new Date(partner.updated_at);
        const hospitalCreated = new Date(hospital.created_at);
        const timeDiff = Math.abs(roleUpdated - hospitalCreated) / 1000; // seconds

        console.log(`   ${profile?.username || 'Unknown'} - ${hospital.hospital_name}`);
        console.log(`     Status: ${hospital.status}, Time diff: ${timeDiff.toFixed(1)}s`);
      });

      // Check for users who should be partners but aren't
      const shouldBePartners = [];
      Object.entries(hospitalsByUserId).forEach(([userId, hospitals]) => {
      ...
      if (shouldBePartners.length > 0) {
        console.log(`\n⚠️  Users with hospitals but not partner role (${shouldBePartners.length}):`);
        shouldBePartners.forEach(user => {
          console.log(`   ${user.username} - Role: ${user.role} (${user.hospitalCount} hospital${user.hospitalCount > 1 ? 's' : ''})`);
        });
      } else {
        console.log('\n✅ All users with hospital applications have correct partner role');
      }

      // Check for partners without hospitals
      const partnersWithoutHospitals = partners.filter(partner => !hospitalsByUserId[partner.user_id]);
      if (partnersWithoutHospitals.length > 0) {
        console.log(`\n⚠️  Partners without hospital applications (${partnersWithoutHospitals.length}):`);
        partnersWithoutHospitals.forEach(partner => {
          const profile = profilesByUserId[partner.user_id];
          console.log(`   ${profile?.username || 'Unknown'} - Partner role but no hospital`);
        });
      }
...
/**
 * hospital Slug Management Functions
 */
class hospitalSlugManager {
  constructor(supabase) {
    this.supabase = supabase;
  }

  async updatehospitalSlugs() {
    console.log('🏷️  Updating hospital Slugs...\n');

    try {
      // First, check current slug status
      const { data: hospitals, error: fetchError } = await this.supabase
        .from('hospitals')
        .select('id, hospital_name, hospital_name_english, slug, created_at')
        .order('created_at', { ascending: false });

      if (fetchError) {
        console.error(`❌ Error fetching hospitals: ${fetchError.message}`);
        return false;
      }

      console.log(`📋 Found ${hospitals.length} hospitals in database`);

      // Identify hospitals without slugs
      const hospitalsWithoutSlugs = hospitals.filter(hospital => !hospital.slug || hospital.slug.trim() === '');
      
      if (hospitalsWithoutSlugs.length === 0) {
        console.log('✅ All hospitals already have slugs!');
        this.displaySlugStatus(hospitals);
        return true;
      }

      console.log(`🔄 Updating ${hospitalsWithoutSlugs.length} hospitals without slugs...`);

      // Update hospitals to trigger slug generation
      for (const hospital of hospitalsWithoutSlugs) {
        ...
        if (updateError) {
          console.error(`❌ Error updating hospital ${hospital.hospital_name}: ${updateError.message}`);
        } else {
          console.log(`✅ Updated: ${hospital.hospital_name}`);
        }
      }

      // Verify updates
      console.log('\n🔍 Verifying slug generation...');
      const { data: updatedHospitals, error: verifyError } = await this.supabase
        .from('hospitals')
        .select('id, hospital_name, hospital_name_english, slug')
        .in('id', hospitalsWithoutSlugs.map(h => h.id));
      ...
      this.displaySlugStatus(updatedHospitals);
      return true;

    } catch (error) {
      console.error(`❌ Error updating hospital slugs: ${error.message}`);
      return false;
    }
  }

  displaySlugStatus(hospitals) {
    console.log('\n📊 hospital Slug Status:');
    console.log('─────────────────────────────────────');
    
    hospitals.forEach(hospital => {
      const hasSlug = hospital.slug && hospital.slug.trim() !== '';
      const status = hasSlug ? '✅' : '❌';
      console.log(`${status} ${hospital.hospital_name} ${hasSlug ? `(${hospital.slug})` : '(no slug)'}`);
    });
    
    const withSlugs = hospitals.filter(g => g.slug && g.slug.trim() !== '').length;
    const withoutSlugs = hospitals.length - withSlugs;
...
class DatabaseUtilities {
  constructor() {
    ...
    this.slugManager = new hospitalSlugManager(this.supabase);
  }
  ...
  async runCommand(command = 'check') {
    ...
    try {
      switch (command.toLowerCase()) {
        ...
        case 'slugs':
        case 'slug':
          return await this.slugManager.updatehospitalSlugs();
        ...
        case 'all':
          ...
          console.log('\n' + '='.repeat(50) + '\n');
          await this.slugManager.updatehospitalSlugs();
          return true;
