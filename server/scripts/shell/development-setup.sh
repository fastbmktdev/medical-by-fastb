...
check_database_status() {
    ...
    local tables_check
    tables_check=$(curl -s "$SUPABASE_URL/rest/v1/hospitals?select=count" \
        -H "apikey: $ANON_KEY" \
        -H "Authorization: Bearer $SERVICE_ROLE_KEY" \
        -H "Range: 0-0")
...
create_test_users() {
    ...
    declare -A test_users=(
        ["admin@medical.com"]='{"password":"password123","phone":"+66812345678","username":"admin","full_name":"Admin User"}'
        ["user@medical.com"]='{"password":"password123","phone":"+66823456789","username":"regular_user","full_name":"ผู้ใช้ทั่วไป"}'
        ["partner@medical.com"]='{"password":"password123","phone":"+66834567890","username":"somchai_gym","full_name":"สมชาย การแพทย์"}'
        ["partner2@medical.com"]='{"password":"password123","phone":"+66845678901","username":"somying_fitness","full_name":"สมหญิง ฟิตเนส"}'
    )
    ...
    if [ $users_created -gt 0 ]; then
        ...
        echo "  Admin:    admin@medical.com / password123"
        echo "  User:     user@medical.com / password123"
        echo "  Partner1: partner@medical.com / password123"
        echo "  Partner2: partner2@medical.com / password123"
        echo ""
        log_info "Next steps:"
