...
import { trackProductView } from "@shared/lib/utils/analytics";
...
export default function ProductDetailPage({
    ...
    async function fetchProduct() {
      try {
        ...
        const productsResponse = await fetch(
          `/api/products?active=true&limit=1000`
        );
        ...
        if (detailData.success) {
          ...
          try {
            const productName =
              detailProduct.nameThai || detailProduct.nameEnglish || "Unknown";
            const category =
              detailProduct.category?.nameThai ||
              detailProduct.category?.nameEnglish ||
              undefined;
            trackProductView(
              detailProduct.id,
              productName,
          ...
          if (detailProduct.variants && detailProduct.variants.length > 0) {
            const defaultVariant = detailProduct.variants.find(
  ...
  const primaryImage =
    product.image || product.images?.[0] || "/assets/images/fallback-img.jpg";
    ...
    <div className="bg-zinc-100 min-h-screen">
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-12 max-w-7xl">
        <div className="gap-12 grid grid-cols-1 lg:grid-cols-2">
          {/* Product Images */}
          <div>
            <div className="relative flex justify-center items-center bg-linear-to-br from-zinc-700 to-zinc-950 mb-4  aspect-square overflow-hidden">
              <Image
                src={primaryImage}
                alt={productName}
                fill
            ...
            {product.images && product.images.length > 1 && (
              <div className="gap-2 grid grid-cols-4">
                {product.images.slice(0, 4).map((img, idx) => (
                  <div
                    key={idx}
                    className="relative aspect-square bg-zinc-800  overflow-hidden cursor-pointer hover:opacity-80 transition-opacity"
                  >
                    <Image
                      src={img}
          ...
          <div>
            ...
            <div className="mb-6">
              {product.category && (
                <span className="inline-block bg-brand-primary mb-3 px-3 py-1  font-semibold text-sm">
                  {product.category.nameThai || product.category.nameEnglish}
                </span>
              )}
              <h1 className="mb-2 font-bold text-4xl">{productName}</h1>
              {product.nameEnglish && product.nameThai && (
                <p className="text-zinc-400 text-lg">{product.nameEnglish}</p>
              )}
            </div>
            ...
            {product.description && (
              <div className="mb-6">
                <h2 className="mb-2 font-semibold text-xl">รายละเอียดสินค้า</h2>
            ...
            {(product.weightKg || product.dimensions) && (
              <div className="mb-6">
                <h2 className="mb-2 font-semibold text-xl">ข้อมูลสินค้า</h2>
                <div className="space-y-1 text-zinc-300">
                  {product.weightKg && (
                    <p>น้ำหนัก: {product.weightKg} กิโลกรัม</p>
                  )}
                  {product.dimensions && <p>ขนาด: {product.dimensions}</p>}
                </div>
              </div>
            )}
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="bg-zinc-100 px-4 py-2 border border-zinc-700  w-20 font-semibold text-center"
                  />
                  <button
                    onClick={() => handleQuantityChange(quantity + 1)}
            ...
            <div className="gap-4 grid grid-cols-1 sm:grid-cols-3 mt-8">
              <div className="bg-zinc-100 p-4 border border-zinc-700  text-center">
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="mb-6 font-bold text-2xl">สินค้าที่เกี่ยวข้อง</h2>
            <div className="gap-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {relatedProducts.map((relatedProduct) => (
                <Link
                  key={relatedProduct.id}
                  href={`/shop/${relatedProduct.slug}`}
                  className="group bg-zinc-100 hover:shadow-lg hover:shadow-red-500/20 border border-zinc-700 hover:border-red-500  overflow-hidden transition-all"
                >
                  <div className="relative aspect-square bg-linear-to-br from-zinc-700 to-zinc-950">
                    <Image
                      src={
                        ...
                        "/assets/images/fallback-img.jpg"
                      }
                      alt={
                        relatedProduct.nameThai ||
                        relatedProduct.nameEnglish ||
                        "Product"
                      }
                      fill
                      sizes="100%"
