...
export default function ArticleCreateModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (metaKeywordsArray.length > 0)
        payload.meta_keywords = metaKeywordsArray;
      ...
      if (formData.og_description)
        payload.og_description = formData.og_description;
      if (formData.canonical_url)
        payload.canonical_url = formData.canonical_url;

      // Add scheduling if provided
      if (formData.scheduled_publish_at) {
        payload.scheduled_publish_at = new Date(
  ...
  // --- CHIP section start ---
  // Compute tags as an array for rendering
        ...
        <ModalBody>
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {formData.image && (
                <div className="mt-2 relative w-full h-32  overflow-hidden">
                  <Image
                    fill
                    sizes='100%'
                    src={formData.image}
            ...
            <div>
              <Input
                ...
                placeholder="การแพทย์, เทคนิค, การฝึก"
                value={formData.tags}
                onChange={(e) =>
              ...
              {tagChips.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {tagChips.map((tag, idx) => (
                    <Chip key={tag + idx} color="primary" variant="flat">
                      {tag}
                    </Chip>
                  ))}
                </div>
            ...
            {showAdvanced && (
              <div className="space-y-4 pt-4 border-t border-zinc-700">
                ...
                />
                {/* Display meta keywords using Chip */}
                {formData.meta_keywords &&
                  formData.meta_keywords
                    .split(",")
                    .map((k) => k.trim())
