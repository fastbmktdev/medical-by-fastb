...
export default function PromotionEditModal({ isOpen, onClose, onSuccess, promotion }: PromotionEditModalProps) {
  ...
  useEffect(() => {
    if (promotion) {
      ...
      setShowDiscountOptions(!!promotion.discount_type);
    }
  }, [promotion]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add coupon code
      if (formData.couponCode !== undefined) {
        payload.couponCode = formData.couponCode.trim() || null;
      }
      ...
      if (formData.minPurchaseAmount !== undefined && formData.minPurchaseAmount) {
        payload.minPurchaseAmount = Number(formData.minPurchaseAmount);
      }
      if (formData.maxDiscountAmount !== undefined && formData.maxDiscountAmount) {
        payload.maxDiscountAmount = Number(formData.maxDiscountAmount);
      }
      if (formData.maxUses !== undefined && formData.maxUses) {
      ...
      <ModalContent>
        ...
        <ModalBody>
          <div className="space-y-4">
            ...
            {/* Discount Fields */}
            {showDiscountOptions && (
              <div className="space-y-4 border border-zinc-700  p-4 bg-zinc-900/50">
                <Select
                  label="ประเภทส่วนลด"
