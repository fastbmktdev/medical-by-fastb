...
import { LeaderboardData } from '@shared/types/gamification.types';
...
export default function LeaderboardWidget({ leaderboard, className = '' }: LeaderboardWidgetProps) {
  ...
  return (
    <div className={`bg-zinc-100/50 backdrop-blur-sm border border-zinc-800/50  p-4 ${className}`}>
      {/* Header */}
            ...
            <div className="text-right">
              ...
              <div className="text-sm text-zinc-500">
                {leaderboard.leaderboard.leaderboard_type === 'points' ? 'คะแนน' :
                 leaderboard.leaderboard.leaderboard_type === 'appointments' ? 'การจอง' :
