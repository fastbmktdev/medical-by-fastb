...
export default function PartnerApplyPage() {
  ...
  const { submitForm, isSubmitting, submitError, errors, setErrors } =
    useFormSubmission({
      supabase,
      user,
      onSuccess: (hospital) => {
    ...
    switch (fieldName) {
      case "hospitalName":
        if (!val || !val.trim()) {
          ...
          return "ชื่อโรงพยาบาลต้องมีอย่างน้อย 3 ตัวอักษร";
        }
        return "";
      ...
      case "hospitalNameEnglish":
        // Optional field, only validate if provided
        if (val && val.trim() && val.trim().length < 3) {
          return "ชื่อโรงพยาบาล (English) ต้องมีอย่างน้อย 3 ตัวอักษร";
        }
        return "";
  ...
  // Show application status if user already has a hospital application
  if (
    existingHospital &&
  ...
  if (existingHospital && applicationStatus === "denied") {
  ...
  return (
    <div className="bg-zinc-100 min-h-screen">
      ...
      <TermsModal
        ...
        hospitalName={formData.hospitalName || "โรงพยาบาลของคุณ"}
      />

      {/* Hero Section */}
      <div className="bg-linear-to-br from-white to-violet-400/40 mt-16">
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-16 max-w-7xl">
          <div className="text-center">
            <h1 className="mb-4 font-semibold text-zinc-950 text-4xl md:text-5xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="pt-4">
            <button
              ...
              className="flex justify-center items-center gap-3 bg-brand-primary hover:bg-red-600 disabled:bg-zinc-600 shadow-lg px-8 py-4  w-full font-semibold text-zinc-950 text-lg hover:scale-[1.02] transition-all disabled:cursor-not-allowed transform"
