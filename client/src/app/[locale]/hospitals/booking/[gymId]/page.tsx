...
export default function HospitalBookingPage() {
  ...
  const [hospitalData, setHospitalData] = useState<{ id: string; hospital_name: string; [key: string]: unknown } | null>(null);
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      // Create hospital appointment record
      await createHospitalBooking(data.orderId, totalAmount, duration);
    } catch (err) {
  ...
  const createHospitalBooking = async (orderId: string, totalAmount: number, duration: number) => {
    if (!selectedPackage) return;

    try {
      const bookingResponse = await fetch('/api/appointments/hospital', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Error creating hospital appointment
    }
  };

  const handlePaymentSuccess = (paymentIntentId: string) => {
    router.push(
      `/hospitals/appointment-success?orderId=${orderId}&orderNumber=${orderNumber}&paymentIntentId=${paymentIntentId}`
    );
  };
      ...
      <div className="flex justify-center items-center bg-gray-50 p-4 min-h-screen">
        <div className="bg-white shadow-lg p-6  w-full max-w-md">
          <div className="text-center">
            ...
            <button
              onClick={() => router.back()}
              className="bg-blue-600 hover:bg-blue-700 px-6 py-2  transition-colors"
            >
              กลับไปหน้าก่อนหน้า
            </button>
          </div>
  ...
  return (
    <div className="bg-gray-50 px-4 sm:px-6 lg:px-8 py-12 min-h-screen">
      <div className="mx-auto max-w-4xl">
        {/* Progress Bar */}
        <div className="bg-white shadow-lg mb-6 p-6 ">
              ...
              <div className="gap-4 grid md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    min={getTomorrowDate()}
                    className="px-4 py-2 border border-gray-300 focus:border-transparent  focus:ring-2 focus:ring-blue-500 w-full"
                  />
                </div>
                <div>
                  <label className="block mb-2 font-medium text-gray-700 text-sm">
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              ...
              <div className="space-y-2 bg-gray-50 p-4  text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">โรงพยาบาล:</span>
                  <span className="font-medium">{typeof hospitalData?.hospital_name === 'string' ? hospitalData.hospital_name : 'Unknown hospital'}</span>
                </div>
                <div className="flex justify-between">
              ...
              {clientSecret && (
                <PaymentWrapper
                  ...
                  returnUrl={`${process.env.NEXT_PUBLIC_APP_URL}/hospitals/appointment-success?orderId=${orderId}&orderNumber=${orderNumber}`}
                  userPhone={contactInfo.phone}
          ...
          {currentStep < 4 && (
            <div className="flex justify-between mt-8">
              <button
                ...
                className="hover:bg-gray-50 disabled:opacity-50 px-6 py-2 border border-gray-300  text-gray-700 transition-colors disabled:cursor-not-allowed"
