                  ...
                  <TableRow key={product.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="relative w-12 h-12 bg-white text-zinc-950  overflow-hidden shrink-0">
                          {product.image ? (
                            <Image
                              src={product.image}
                              alt={
                    ...
                    <TableCell>
                      {product.category ? (
                        <Chip variant="flat" size="sm">
                          {product.category.nameThai ||
                            product.category.nameEnglish}
                        </Chip>
                      ) : (
                    ...
                    <TableCell>{getStockChip(product.stock || 0)}</TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-1">
                        <Chip
                          color={
                            product.isActive !== false ? "success" : "danger"
                          }
                          variant="flat"
                          size="sm"
                        >
          ...
          <ProductVariantsModal
            ...
            onSave={async (variants) => {
              try {
                ...
                const currentVariants = currentData.success
                  ? currentData.data
                  : [];
                ...
                for (const currentVariant of currentVariants) {
                  if (!variants.find((v) => v.id === currentVariant.id)) {
                    await fetch(
                      `/api/products/${selectedProduct.id}/variants/${currentVariant.id}`,
                      {
                        method: "DELETE",
                      }
                    );
                ...
                for (const variant of variants) {
                  if (variant.id) {
                    ...
                    await fetch(
                      `/api/products/${selectedProduct.id}/variants/${variant.id}`,
                      {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
          ...
          <ProductImagesModal
            ...
            onSave={async (images) => {
              try {
                ...
                // Get current images
                const currentResponse = await fetch(
                  `/api/products/${selectedProduct.id}/images`
                );
                const currentData = await currentResponse.json();
                const currentImages = currentData.success
                  ? currentData.data
                  : [];
                ...
                for (const currentImage of currentImages) {
                  if (!images.find((img) => img.id === currentImage.id)) {
                    await fetch(
                      `/api/products/${selectedProduct.id}/images/${currentImage.id}`,
                      {
                        method: "DELETE",
                      }
                    );
