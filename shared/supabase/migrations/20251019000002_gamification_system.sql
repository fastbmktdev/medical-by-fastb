...
-- ---
-- This migration creates a comprehensive gamification system for the Medical platform
...
CREATE TABLE IF NOT EXISTS points_history (
  ...
  reference_id UUID, -- Can reference appointments, articles, etc.
  reference_type TEXT, -- 'appointment', 'article', 'review', etc.
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc', NOW())
);
-- ---
...
CREATE TABLE IF NOT EXISTS badges (
  ...
  category TEXT NOT NULL, -- 'appointment', 'loyalty', 'social', 'learning', 'special'
...
CREATE TABLE IF NOT EXISTS user_streaks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  streak_type TEXT NOT NULL, -- 'appointment', 'login', 'review', 'article_read'
...
CREATE TABLE IF NOT EXISTS leaderboards (
  ...
  name_english TEXT,
  description TEXT,
  leaderboard_type TEXT NOT NULL, -- 'points', 'appointments', 'streak', 'monthly', 'all_time'
...
CREATE TABLE IF NOT EXISTS gamification_rules (
  ...
  action_type TEXT NOT NULL, -- 'appointment', 'review', 'login', 'article_read', etc.
...
-- appointment badges
('นักชกหน้าใหม่', 'Rookie patient', 'จองโรงพยาบาลครั้งแรก', 'First hospital appointment', 0, 'appointment', 'common'),
('นักชกประจำ', 'Regular patient', 'จองโรงพยาบาล 5 ครั้ง', 'Booked hospital 5 times', 250, 'appointment', 'common'),
('นักชกมืออาชีพ', 'Professional patient', 'จองโรงพยาบาล 20 ครั้ง', 'Booked hospital 20 times', 1000, 'appointment', 'rare'),
('นักชกแชมป์', 'Champion patient', 'จองโรงพยาบาล 50 ครั้ง', 'Booked hospital 50 times', 2500, 'appointment', 'epic'),
...
('นักแชร์', 'Social Sharer', 'แชร์โรงพยาบาล 10 ครั้ง', 'Shared hospital 10 times', 100, 'social', 'common'),
...
('นักชกแห่งปี', 'patient of the Year', 'ได้คะแนนสูงสุดในปี', 'Highest points in the year', 2000, 'special', 'legendary');

-- Insert default gamification rules
INSERT INTO gamification_rules (rule_name, action_type, points_awarded, conditions) VALUES
-- appointment rules
('first_booking', 'appointment', 100, '{"is_first_booking": true}'),
('regular_booking', 'appointment', 50, '{"package_type": "one_time"}'),
('package_booking', 'appointment', 75, '{"package_type": "package"}'),
('long_term_package', 'appointment', 150, '{"duration_months": {"gte": 6}}'),
...
('การจองมากที่สุด', 'Most appointments', 'ผู้เล่นที่จองโรงพยาบาลมากที่สุด', 'appointments', 'all_time'),
('สตรีคการจอง', 'appointment Streak', 'ผู้เล่นที่มีสตรีคการจองยาวนานที่สุด', 'streak', 'all_time');
-- ---
