...
CREATE TABLE IF NOT EXISTS user_favorites (
  ...
  item_type TEXT NOT NULL CHECK (item_type IN ('hospital', 'product', 'event')),
...
-- ---
-- Function to get user favorites with hospital details
...
BEGIN
  ...
  LEFT JOIN hospitals g ON uf.item_type = 'hospital' AND uf.item_id = g.id AND g.status = 'approved'
...
-- PART 7: COMMENTS
-- ---
COMMENT ON TABLE user_favorites IS 'Stores user favorites (hospitals, products, events)';
...
COMMENT ON COLUMN user_favorites.item_type IS 'Type of favorited item: hospital, product, or event';
COMMENT ON FUNCTION get_user_favorites_with_details IS 'Get user favorites with hospital details';
