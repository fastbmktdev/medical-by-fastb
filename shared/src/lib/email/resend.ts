    ...
    <html>
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">🥊 medical Platform</h1>
        ...
        <div style="text-align: center; margin-top: 20px; padding: 20px;">
          <p style="color: #6b7280; font-size: 12px; margin: 0;">
            Email sent automatically from medical Platform Contact Form
          </p>
        </div>
      </body>
    </html>
...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  try {
    const result = await resend.emails.send({
      ...
      html: `
        <h2>สวัสดี ${name}!</h2>
        <p>ยินดีต้อนรับสู่ medical Platform</p>
        <p>เราดีใจที่คุณมาร่วมเป็นส่วนหนึ่งของชุมชนการแพทย์</p>
        <p>เริ่มต้นสำรวจ:</p>
        <ul>
          <li>ค้นหาโรงพยาบาลใกล้คุณ</li>
          <li>เรียนรู้เทคนิคการแพทย์</li>
...
function generateVerificationEmailHtml(data: { otp: string; fullName: string }): string {
  ...
  return `
    ...
    <html>
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 28px;">🥊 medical Platform</h1>
        ...
        <div style="text-align: center; margin-top: 30px; padding: 20px;">
          ...
          <p style="margin: 0;">
            <a href="mailto:support@medical.com" style="color: #dc2626; text-decoration: none; font-size: 14px;">
              support@medical.com
            </a>
          </p>
        </div>
...
export async function sendBookingConfirmationEmail(
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `ยืนยันการจองสำเร็จ - ${data.bookingNumber} | medical Platform`,
...
export async function sendBookingReminderEmail(data: BookingReminderData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `📅 เตือนความจำ: การจองของคุณจะเริ่มในอีก 1 วัน | medical Platform`,
...
export async function sendEventReminderEmail(data: EventReminderData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `🎫 เตือนความจำ: อีเวนต์ของคุณจะเริ่มในอีก 1 วัน | medical Platform`,
...
export async function sendPaymentReceiptEmail(data: PaymentReceiptData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `ใบเสร็จการชำระเงิน - ${data.transactionNumber} | medical Platform`,
...
export async function sendPaymentFailedEmail(data: PaymentFailedData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `การชำระเงินไม่สำเร็จ - ${data.transactionNumber} | medical Platform`,
...
export async function sendPartnerApprovalEmail(data: PartnerApprovalData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `🎉 ยินดีด้วย! การสมัครโรงพยาบาลของคุณได้รับการอนุมัติ | medical Platform`,
...
export async function sendPartnerRejectionEmail(data: PartnerRejectionData) {
  ...
  try {
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: `การสมัครโรงพยาบาลของคุณยังไม่ได้รับการอนุมัติ | medical Platform`,
...
export async function sendAdminAlertEmail(data: AdminAlertData) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: recipients,
      subject: `${priorityEmoji[data.priority || 'medium']} [${data.priority?.toUpperCase() || 'MEDIUM'}] ${data.title} | medical Platform`,
...
export async function sendPasswordResetEmail(data: PasswordResetEmailData) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: data.to,
      subject: 'รีเซ็ตรหัสผ่าน - medical Platform 🥊',
      html: htmlContent,
    });
